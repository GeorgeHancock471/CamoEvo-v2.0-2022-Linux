...
"RNL Map",							//0 //Title
"8", 								//1 //Scale
"2",								//2 //Resolution(axis JND)
"Colour based on map location",		//3 //Pallette
"false",							//4 //Unify Colours
"1.0",								//5 //Boundary Threshold
"1",								//6 // JND perimiter size
"20",								//7 // Border Size
"2",								//8 // Tick Length
"2",								//9 // Line width
"4",								//10 // Axis Font Size
"true",								//11 // Map Labels
"3"									//12 // Label Font Size								
)

defaultDIR = getDirectory("plugins")+"/micaToolbox/RNL Colour Maps/RNL_Settings/Plot&Compare.txt"
...
Dialog.create("Colour Map Plotting Settings");
	...
	Dialog.addString("Figure title", settingsDefault[0], 20);
	Dialog.addNumber("Scale", parseFloat(settingsDefault[1]));
	Dialog.addMessage("The colour-map resolution is multiplied by this, higher\nvalues produces higher-resolution figures.");
	Dialog.addNumber("Z-Axis_resolution (JNDs)", parseFloat(settingsDefault[2]));
	
	Dialog.addChoice("Colour options", colOptions, settingsDefault[3]);
	Dialog.addCheckbox("Unify colours within maps", settingsDefault[4]);
	
	Dialog.addNumber("Boundary threshold x10^6",parseFloat(settingsDefault[5]));
	Dialog.addNumber("JND Perimeter Size", parseFloat(settingsDefault[6]));
	Dialog.addNumber("Figure border size", parseFloat(settingsDefault[7]));
	Dialog.addNumber("Tick length", parseFloat(settingsDefault[8]));
	Dialog.addNumber("Line width", parseFloat(settingsDefault[9]));
	Dialog.addNumber("Axis_font size", parseFloat(settingsDefault[10]));
	Dialog.addCheckbox("Add colour map labels", settingsDefault[11]);	
	Dialog.addNumber("Label font size", settingsDefault[12]);
...
settingsDefault[0]=figTitle;
settingsDefault[1]=scaleFactor;
settingsDefault[2]=zRes;
settingsDefault[3]=colSel;
settingsDefault[4]=mapColours;
settingsDefault[5]=minThreshold;
settingsDefault[6]=jndBoundary;
settingsDefault[7]=border; 
settingsDefault[8]=tick; 
settingsDefault[9]=lineWidth;
settingsDefault[10]=axesFont; 
settingsDefault[11]=addLabels;
settingsDefault[12]=labelFont; 
